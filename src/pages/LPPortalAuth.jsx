...
import {
  ...
  Loader2, Bell, Clock, Download, LogOut,
  PieChart, Wallet, ArrowUpRight, ArrowDownRight, MessageSquare
} from 'lucide-react';
...
export default function LPPortalAuth() {
  const navigate = useNavigate();
  const { user, authToken, logout } = useAuth();

  // Fetch LP's investments via authenticated API
  ...
  const handleInvestmentClick = (investment) => {
    navigate(`/LPInvestmentDetail?dealId=${investment.dealId || investment.id}`);
  };
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">{user?.organization?.name || 'Canonical Capital'}</p>
              </div>
            </div>
              ...
              <div className="grid gap-4">
                {investments.map((investment) => (
                  <InvestmentCard
                    ...
                    investment={{
                      id: investment.dealId || investment.id,
                      name: investment.entityName || investment.dealName,
                      asset_type: investment.assetType || 'Real Estate',
                      status: investment.dealStatus || 'DD_ACTIVE',
                      key_notes: investment.notes
                    }}
                    lpActor={investment}
      ...
      <footer className="bg-white border-t border-gray-200 mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <p className="text-center text-sm text-gray-500">
            {user?.organization?.name || 'Canonical Capital'} LP Portal • Questions? Contact your investment manager
          </p>
        </div>
      </footer>
    </div>
